...
Photo-message разрешает `image` и локализованный `alt` по тому же `sourceId`. Поэтому уже полученное изображение остаётся тем же после RU ↔ EN, а подпись и доступный alt меняются вместе с остальным transcript.

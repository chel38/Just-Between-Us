...
const zip = new JSZip();
const maxUncompressedBytes = 100_000_000;
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await addDirectory(fullPath);
      ...
      if (uncompressedBytes >= maxUncompressedBytes) {
        throw new Error(`Release must be smaller than 100 MB uncompressed (current: ${uncompressedBytes} bytes).`);
      }
...
console.log(`Created ${outputFile} (${uncompressedBytes} bytes uncompressed)`);
